...
public class MainActivityRedefinirSenha extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnConfirmarEmail = findViewById(R.id.btnConfirmarEmail);
        btnAutenticar = findViewById(R.id.btnAutenticar);
        editTitular = findViewById(R.id.editTitular);
        ...
        btnAutenticar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //busca_senha(intent.getStringExtra("cartao"));
                entrar_por_email_senha(email_app,senha_app);
        ...
        btnRedefinir.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                redefinirSenha(mUser,novasenha,email_destino,matricula,empregador);
    ...
    public boolean onSupportNavigateUp() {
        if(isTaskRoot()){
            ...
            Intent intent = new Intent(MainActivityRedefinirSenha.this, Activity_tela_User.class);
            startActivity(intent);
            return true;
        }
    }
    private void busca_senha(String cartao) {
        String urlm = getResources().getString(R.string.HOST) + "localiza_senha_associado_app.php";
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, urlm,
                response -> {
                    ...
                    try {
                        ...
                        String existe_cartao;
                        existe_cartao = campos.getString("existe_cartao");
                        if(existe_cartao.equals("true")) {
                            email_app = campos.getString("email");
                            senha_app = campos.getString("senha");
                            matricula = campos.getString("cod_associado");
                            empregador = campos.getString("empregador");
                            nome_titular = campos.getString("nome");
                            if (editEmailRedefinir.getText().toString().trim().equals(email_app.trim())) {
                                editTitular.setText(nome_titular);
                                ...
                                AestheticDialog.Builder dialog1 =
                                        new AestheticDialog.Builder(MainActivityRedefinirSenha.this,
                                ...
                                dialog1.setOnClickListener(new OnDialogClickListener() {
                                    ...
                                    public void onClick(@NonNull AestheticDialog.Builder builder) {
                                        ...
                                        dialog1.setTitle("Atenção!");
                                        dialog1.setMessage("Click no botão Confirmar o nome do titular!");
                                        dialog1.setCancelable(false);
                                        dialog1.setDarkMode(true);
                                        dialog1.setGravity(Gravity.CENTER);
                                        dialog1.setAnimation(DialogAnimation.SHRINK);
                            ...
                            dialog1.setTitle("Atenção!");
                            dialog1.setMessage("O cartão está incorreto !!!");
                            dialog1.setCancelable(false);
                            dialog1.setDarkMode(true);
                            dialog1.setGravity(Gravity.CENTER);
                            dialog1.setAnimation(DialogAnimation.SHRINK);
                            dialog1.setOnClickListener(new OnDialogClickListener() {
                                @Override
                                public void onClick(@NonNull AestheticDialog.Builder builder) {
                                    Intent intent = new Intent(MainActivityRedefinirSenha.this, MainActivity.class);
                                    startActivity(intent);
                                }
                            });
                            dialog1.show();
            ...
            Toast.makeText(MainActivityRedefinirSenha.this, message, Toast.LENGTH_LONG).show();
        }) {
            @Override
    ...
    private void reautenticar(FirebaseUser firebaseUser, String email_app) {
        if(senha_assoc != null) {
            entrar_por_email_senha(email_app, senha_assoc);

            AuthCredential credential = EmailAuthProvider.getCredential(email_app, senha_app);
            firebaseUser.reauthenticate(credential).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        //Toast.makeText(MainActivityRedefinirSenha.this, "User re-authenticated.",
                        //        Toast.LENGTH_SHORT).show();
                        AestheticDialog.Builder dialog1 =
                                new AestheticDialog.Builder(MainActivityRedefinirSenha.this,
                                        DialogStyle.FLAT, DialogType.SUCCESS);
                        dialog1.setTitle("Atenção!");
                        dialog1.setMessage("O nome do titular foi comfirmado com sucesso!");
                        dialog1.setCancelable(false);
                        dialog1.setDarkMode(true);
                        dialog1.setGravity(Gravity.CENTER);
                        dialog1.setAnimation(DialogAnimation.SHRINK);
    ...
    private void redefinirSenha(FirebaseUser firebaseUser,String novasenha,String emaildestino,String matricula,String empregador) {
        firebaseUser.updatePassword(novasenha).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()) {
                    atualiza_senha_associado(matricula,empregador,novasenha);
                    ...
                    try{
                        ...
                        Toast.makeText(MainActivityRedefinirSenha.this, e.getMessage(), Toast.LENGTH_LONG).show();
                    }
                }
            }
        });
    }
    public void atualiza_senha_associado(String matricula,String empregador, String senha) {
        String urlm = getResources( ).getString(R.string.HOST) + "atualiza_senha_app.php";
        requestQueue = Volley.newRequestQueue(MainActivityRedefinirSenha.this);
        StringRequest stringRequest = new StringRequest(Request.Method.POST,urlm,
                response -> {
                }, error -> {
        }) {
    ...
    public void SendEmail(String senha_x, String Email_destino){
        ...
        try {
            ...
            thread.start();
            //Toast.makeText(MainActivityRedefinirSenha.this, "Email enviado com sucesso", Toast.LENGTH_LONG).show();
        } catch (MessagingException e) {
            e.printStackTrace();
        }
    }
